...
// ─── Carte certificat ─────────────────────────────────────────────────────────
function CertificateCard({ cert, fullName, isSelected, onClick }) {
...
export default function CertificatesPage() {
  ...
  const cert = certificates[selected]
  const moduleTitle = cert.modules?.title || 'AI Foundations 101'
  const date = new Date(cert.issued_at).toLocaleDateString('fr-FR', {
    day: 'numeric', month: 'long', year: 'numeric'
  })
        ...
        <div className="w-full lg:w-[40%] flex flex-col gap-4">
          ...
          <div className="bg-white rounded-2xl p-6 border border-[#8127cf]/10 shadow-sm">
            ...
            <div className="space-y-3">
              {[
                { label: 'Module',         value: moduleTitle,  type: 'text'  },
